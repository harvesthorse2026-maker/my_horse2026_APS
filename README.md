[index.html](https://github.com/user-attachments/files/24284386/index.html)
<!DOCTYPE html>
<html lang="zh-Hant">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>小強“賽馬研究所“</title>
    <style>
        /* --- 基礎設定 --- */
        :root {
            --primary-bg: #f2f4f8;
...
<body>
    ...
    <header class="app-header">
        ...
        <div class="header-title" id="pageTitle">小強“賽馬研究所“</div>
    ...
    <div id="page-detail" class="page">
        <div class="detail-header-info">
            <span id="detail-race-no">R?</span> - 小強機會提供
        </div>
        <div class="horse-list-header">
            <div>馬匹資料</div>
            <div style="text-align:center">賠率</div>
            <div style="text-align:center">賠率排序</div>
            <div style="text-align:center">走位</div>
        </div>
    ...
    <div class="bottom-nav">
        <div class="nav-item active"><span class="nav-icon">📊</span>統計</div>
        <div class="nav-item"><span class="nav-icon">🐎</span>馬匹</div>
        <div class="nav-item"><span class="nav-icon">👤</span>用戶</div>
    </div>

    <script>
        // ============================================
        // 🔴 請在這裡貼上你的 Google Script 網址
        const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzITGOiw4hgVdvfYgDJgyHugojYQ1G4YXy-_7fn2OeCQuvPeL4UJm0JQTYHoDsCqrA6/exec";
        // ============================================

        let globalData = []; // 存儲下載的資料
        ...
        window.onload = async function() {
            try {
                ...
                alert("資料讀取失敗，請檢查網路或 Script 網址。\n" + error);
                document.getElementById('loading-mask').innerText = "讀取失敗";
            }
        };
            ...
            data.forEach(race => {
                ...
                div.innerHTML = `
                    ...
                    <div class="race-card-body">
                        <div>
                            <div class="stat-row"><span class="stat-label">評分:</span> ${race.rating}</div>
                        </div>
                        <div>
                            <div class="stat-row"><span class="stat-label">主力提供:</span> <span class="stat-value">${race.pick}</span></div>
                            <div class="stat-row"><span class="stat-label">主力彩池:</span> <span class="stat-value">${race.pool}</span></div>
                            <div class="color-bar">
                                ${colorsHtml}
                            </div>
                        </div>
        ...
        function goToRace(raceId) {
            ...
            document.getElementById('pageTitle').innerText = raceId + " 賽事詳情";
        ...
        function goHome() {
            ...
            document.getElementById('pageTitle').innerText = '小強“賽馬研究所“';
        }
        ...
        function renderHorses(horses) {
            ...
            // 這裡已經是後端排好序 (Column K) 的資料了，直接渲染即可
            horses.forEach(h => {
                ...
                card.innerHTML = `
                    <div class="h-main-row">
                        <div class="h-info-group">
                            ...
                            <div class="h-name-box">
                                ...
                                <div class="h-sub-info">${h.trainer} | ${h.jockey} | ${h.draw}檔 | ${h.weight}磅</div>
                            </div>
                        </div>
                    ...
                    <div class="h-details">
                        <div class="h-details-content">
                            <span class="comment-label">機會短評 (M欄)：</span>
                            ${comment}
                            
                            ${reason ? `
                            <div class="reason-section">
                                <span class="reason-label">詳細分析 (N欄)：</span><br>
                                ${reason}
                            </div>` : ''}
                        </div>
                    </div>
